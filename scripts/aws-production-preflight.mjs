...
const required = [
  ["CAFA_PRODUCTION_APPROVED_REGION", "approved AWS region"],
  ["CAFA_PRODUCTION_HOSTNAME", "approved HTTPS hostname"],
  ["CAFA_PRODUCTION_CERTIFICATE_ARN", "ACM certificate ARN"],
];

function fail(message) {
  console.error(`AWS production preflight blocked: ${message}`);
  process.exit(1);
}
...
const approvedRegion = value("CAFA_PRODUCTION_APPROVED_REGION");
const hostname = value("CAFA_PRODUCTION_HOSTNAME");
const certificateArn = value("CAFA_PRODUCTION_CERTIFICATE_ARN");
...
if (
  ...
  fail("the production hostname must be a bare hostname, not a URL, wildcard, or path.");
}

if (!/^arn:[^:]+:acm:[^:]+:[0-9]{12}:certificate\/[A-Za-z0-9-]+$/.test(certificateArn)) {
  fail("the production certificate ARN is malformed.");
}
...
if (certificate?.Status !== "ISSUED" || !certificateCoversHost) {
  fail("the ACM certificate is not issued for the approved production hostname.");
}
...
try {
  execFileSync(
    "aws",
    ["cloudformation", "describe-stacks", "--region", approvedRegion, "--stack-name", "cafa-pmis-production"],
    { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] },
  );
} catch (error) {
  const detail = error.stderr?.toString() ?? "";
  if (!/does not exist|ValidationError/i.test(detail)) {
    fail("the deployment identity cannot verify access to the production CloudFormation stack.");
  }
}

console.log(`AWS production preflight passed: identity=${identity}, region=${approvedRegion}, az_count=${azs.length}`);
