 ...
 * reviewer comments. The comments.section column stores one of these keys
 * (or NULL, which renders as "General / Report-Level").
 *
 * Keys mirror the sections actually rendered by the SPR form and the
 * submitted-detail view (program-state-report-form.tsx). Do not add keys
 * that have no corresponding rendered section.
 */
...
export function isSprSectionKey(value: string): value is SprSectionKey {
  return (SPR_SECTION_KEYS as readonly string[]).includes(value);
}

export function getSprSectionLabel(key: string): string {
  return isSprSectionKey(key) ? SPR_SECTION_LABELS[key] : key;
}
